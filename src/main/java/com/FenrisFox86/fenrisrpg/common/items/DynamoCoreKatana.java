...
public class DynamoCoreKatana extends SingleHandedSwordItem {
    ...
    public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if(InputMappings.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            tooltip.add(new TranslationTextComponent("tooltip.fenrisrpg.dynamo_core_katana.lore"));
            tooltip.add(new TranslationTextComponent("tooltip.fenrisrpg.dynamo_core_katana.desc"));
        } else {
