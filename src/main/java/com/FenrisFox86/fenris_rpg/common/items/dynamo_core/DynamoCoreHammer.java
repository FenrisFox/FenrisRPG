...
import com.FenrisFox86.fenris_rpg.common.items.HammerItem;
...
public class DynamoCoreHammer extends HammerItem {
    ...
    public void appendHoverText(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if(InputMappings.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            tooltip.add(new TranslationTextComponent("tooltip.fenris_rpg.dynamo_core_hammer.lore"));
            tooltip.add(new TranslationTextComponent("tooltip.fenris_rpg.dynamo_core_hammer.desc"));
        } else {
    ...
    public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand handIn) {
        if(!playerIn.getCooldowns().isOnCooldown(this)) {
            playerIn.addEffect(new EffectInstance(Effects.DIG_SPEED, 500, 3));
